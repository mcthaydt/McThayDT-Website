...
export class DatabaseStorage implements IStorage {
  ...
  async seed(): Promise<void> {
    ...
    // Prompts
    const existingPrompts = await db.select({ count: sql<number>`count(*)` }).from(prompts);
    if (Number(existingPrompts[0].count) === 0) {
    ...
    // Fact Sheet Items
    const existingFacts = await db.select({ count: sql<number>`count(*)` }).from(factSheetItems);
    if (Number(existingFacts[0].count) === 0) {
      console.log("Seeding fact sheet items...");
      const funFacts = {
        games: [
    ...
    // Psychographics
    const existingPsychographics = await db.select({ count: sql<number>`count(*)` }).from(psychographics);
    if (Number(existingPsychographics[0].count) === 0) {
      console.log("Seeding psychographics...");
